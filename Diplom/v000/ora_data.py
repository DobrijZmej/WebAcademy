...
class ora_data:
    ...
    def __init__(self):
        try:
            self.connect = cx_Oracle.connect('c##dobrijzmej', '7014258', 'ZMEJ')
    ...
    def get_missions_on_day(self, in_cmdr_nick):
        """Возвращает количество сделанных миссий всеми пилотами"""
        sql = """/* get_missions_on_day */
select t.cmdr,
       decode(t.faction, 'Ukraine Colonist Alliance', 'UCA',
                         'Eol Prou Group',            'EPG',
              t.faction)
           ||' -> '|| 
        decode(t.dest_faction, 'Ukraine Colonist Alliance', 'UCA',
                               'Eol Prou Group',            'EPG',
              t.dest_faction) faction,
       listagg(decode(t.influence, 'Low', '+', 'Med', '++', 'High', '+++', t.influence), ',') WITHIN GROUP (order by t.id) rrr,
       sum(1*decode(t.influence, 'Low', 1, 'Med', 2, 'High', 3)) count_missions2,
       count(t.id) count_missions,
       sum(t.reward) reward
from   UCA_MISSIONS t
where  t.date_complete >= trunc(sysdate)
       and (:cmdr is null or lower(t.cmdr) = lower(:cmdr))
group by t.cmdr,
       decode(t.faction, 'Ukraine Colonist Alliance', 'UCA',
                         'Eol Prou Group',            'EPG',
              t.faction)
        ...
        decode(t.dest_faction, 'Ukraine Colonist Alliance', 'UCA',
                         'Eol Prou Group',            'EPG',
              t.dest_faction)
order by t.cmdr, count_missions desc,
       decode(t.faction, 'Ukraine Colonist Alliance', 'UCA',
                         'Eol Prou Group',            'EPG',
              t.faction)
           ||' -> '|| 
        decode(t.dest_faction, 'Ukraine Colonist Alliance', 'UCA',
              ...
              t.dest_faction)
                   """
        cur = self.connect.cursor()
        cur.execute(sql, cmdr=in_cmdr_nick)
        result = rows_to_dict_list(cur)
        #print(result)
        return result

    def get_bountys_on_day(self, in_cmdr_nick):
        """Возвращает количество дочерних объектов тела"""
        sql = """/* get_bountys_on_day */
select   t.cmdr,
         json_value(t.entry, '$[0].Faction')||json_value(t.entry, '$[0].Factions.Faction') Factions,
         sum(json_value(t.entry, '$[0].Amount')) Amount
from     UCA_EVENTS t
where    json_value(t.entry, '$[0].event') in ('RedeemVoucher')
         and t.sys_date>=trunc(sysdate)
         and (:cmdr is null or lower(t.cmdr) = lower(:cmdr))
group by t.cmdr,
         json_value(t.entry, '$[0].Faction')||json_value(t.entry, '$[0].Factions.Faction')
order by Amount desc
    ...
    def get_ship_kills_on_day(self, in_cmdr_nick):
        """Возвращает количество сбитых кораблей"""
        sql = """/* get_ship_kills_on_day */
select   t.cmdr,
         json_value(t.entry, '$[0].Target') Target,
         count(json_value(t.entry, '$[0].Target')) TargetCount,
         sum(json_value(t.entry, '$[0].TotalReward')) TotalReward
from     UCA_EVENTS t
where    json_value(t.entry, '$[0].event') = ('Bounty')
         and t.sys_date>=trunc(sysdate)
         and (:cmdr is null or lower(t.cmdr) = lower(:cmdr))
group by t.cmdr,
         json_value(t.entry, '$[0].Target')
union
select   t.cmdr,
         'Faction '||json_value(t.entry, '$[0].VictimFaction') Target,
         count(json_value(t.entry, '$[0].VictimFaction')) TargetCount,
         sum(json_value(t.entry, '$[0].Reward')) TotalReward
from     UCA_EVENTS t
where    json_value(t.entry, '$[0].event') = ('FactionKillBond')
         and t.sys_date>=trunc(sysdate)
         and (:cmdr is null or lower(t.cmdr) = lower(:cmdr))
group by t.cmdr,
         'Faction '||json_value(t.entry, '$[0].VictimFaction')
order by TotalReward desc, TargetCount desc, Target
                   """
        cur = self.connect.cursor()
        cur.execute(sql, cmdr=in_cmdr_nick)
        result = rows_to_dict_list(cur)
    ...
    def get_distance_on_day(self, in_cmdr_nick):
        """Возвращает количество прыжков за день"""
        sql = """/* get_distance_on_day */
select   count(t.id) JumpsCount,
         sum(replace(json_value(t.entry, '$[0].JumpDist'), '.', ',')) JumpDist,
         sum(replace(json_value(t.entry, '$[0].FuelUsed'), '.', ',')) FuelUsed
from     UCA_EVENTS t
where    1=1
         and json_value(t.entry, '$[0].event') in ('FSDJump') -- припрыг в систему
         and t.sys_date>=trunc(sysdate)
         and (:cmdr is null or lower(t.cmdr) = lower(:cmdr))
                   """
        cur = self.connect.cursor()
